...
DATASET=Akshit03/AkshitMajorProjectMIR1_cube_strawberry_combined
LOG_DIR=logs
mkdir -p "$LOG_DIR"
OUT_ROOT=/media/jetson-agx/SANDISK1/outputs/train/smolvla_mir1_strawberry
...
logfile="$LOG_DIR/smolvla_mir1_strawberry.log"

echo ">>> Starting smolvla_mir1_strawberry at $(date)"
...
lerobot-train \
    ...
    --policy.device=cuda \
    --policy.repo_id=Akshit03/smolvla_mir1_strawberry \
    --output_dir="$OUT_ROOT" \
...
mins=$(( (t1 - t0) / 60 ))
echo ">>> Finished smolvla_mir1_strawberry rc=$rc in ${mins}m at $(date)"
df -h / /media/jetson-agx/SANDISK1 | tail -2

final_loss=$(grep -oE "loss:[0-9.]+" "$logfile" | tail -1)
echo "SMOLVLA_STRAWBERRY DONE rc=$rc mins=$mins final_loss=${final_loss:-n/a}"
